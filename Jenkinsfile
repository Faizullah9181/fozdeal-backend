    ...
    stages{
        stage("Checkout branch"){
            steps{
                ...
                git branch: env.BRANCH_NAME ,credentialsId:'gitcreds',url:'https://github.com/Orca-Brand/orca-boilerplate.git'
            }
        }
        ...
        stage("Get configurations"){
            steps{
                ...
                sh "rm -rf orca-configurations; mkdir -p orca-configurations; cd orca-configurations"
                dir("orca-configurations"){
                    git branch: "master", credentialsId: "gitcreds", url: "https://github.com/Orca-Brand/orca-configurations.git"
                }
                sh "mv orca-configurations/${SERVICE}/${ENVIRONMENT}/.env .env"
                sh "mv orca-configurations/${SERVICE}/${ENVIRONMENT}/firebase.json firebase.json"
                load ".env"
            }
        }
        stage('Test Suit'){
